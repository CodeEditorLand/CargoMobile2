...
impl Action {
	...
	pub fn dest(&self) -> &Path {
		match self {
			Self::CreateDirectory { dest } | Self::CopyFile { dest, .. } | Self::WriteTemplate { dest, .. } => dest,
		}
	}
}
...
fn traverse_dir<E: Debug + Display + StdError>(
	...
	if src.is_file() {
		...
		for entry in fs::read_dir(src).map_err(|cause| TraversalError::DirectoryRead { path: src.to_owned(), cause })? {
			let path = entry
				.map_err(|cause| TraversalError::EntryRead { dir: src.to_owned(), cause })?
				.path();

			if path.is_dir() {
				traverse_dir(&path, &append_path(dest, &path, false), transform_path, template_ext, actions)?;
			} else {
				actions.push_back(
					file_action(&path, dest, transform_path, template_ext)
						.map_err(|cause| TraversalError::PathTransform { path: path.to_owned(), cause })?,
				);
			}
		}
	}
