...
impl DotCargo {
	...
	pub fn load(app: &App) -> Result<Self, LoadError> {
		let path =
			Self::create_dir_and_get_path(app).map_err(|(path, cause)| LoadError::DirCreationFailed { path, cause })?;
		...
		if path.is_file() {
			let toml_str =
				fs::read_to_string(&path).map_err(|cause| LoadError::ReadFailed { path: path.clone(), cause })?;
