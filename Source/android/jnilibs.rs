	...
	fn report(&self) -> Report {
		match self {
			Self::ReadDir { dir, source } => {
				Report::error(format!("Failed to list contents of jniLibs directory {:?}", dir), source)
			},
...
impl JniLibs {
	...
	pub fn remove_broken_links(config: &Config) -> Result<(), RemoveBrokenLinksError> {
			...
			.filter(|path| path.is_dir())
		{
			...
			{
				let entry = entry
					.map_err(|source| RemoveBrokenLinksError::Entry { dir: abi_dir.clone(), source })?
					.path();
	...
	pub fn symlink_lib(&self, src: &Path) -> Result<(), SymlinkLibError> {
		...
		if src.is_file() {
			let dest = self.path.join(src.file_name().expect("developer error: file had no file name"));

			os::ln::force_symlink(src, dest, ln::TargetStyle::File).map_err(SymlinkLibError::SymlinkFailed)?;
