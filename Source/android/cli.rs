...
impl Exec for Input {
	...
	fn exec(self, wrapper: &TextWrapper) -> Result<(), Self::Report> {
				...
				env.base = env.base.explicit_env_vars(
					vars.iter()
						.map(|d| {
							(
								d.0.to_owned(),
								OsString::from(d.1.replace(
									"<android-project-dir>",
									&dunce::simplified(&config.android().project_dir()).to_string_lossy(),
								)),
							)
						})
		...
		fn open_in_android_studio(config: &Config, env: &Env) -> Result<(), Error> {
			os::open_file_with("Android Studio", config.project_dir(), &env.base).map_err(Error::OpenFailed)
		}
		...
		match command {
			...
			Command::Check { targets } => with_config(non_interactive, wrapper, |config, metadata, env| {
			...
			Command::Run {
				...
				device_prompt(env)
					...
					.run(
						...
						reinstall_deps,
						activity
							.unwrap_or_else(|| metadata.app_activity_name().unwrap_or(DEFAULT_ACTIVITY).to_string()),
					)
			...
			Command::Apk { cmd } => match cmd {
				ApkSubcommand::Build { targets, profile: cli::Profile { profile }, split_per_abi } => {
					with_config(non_interactive, wrapper, |config, _, env| {
						ensure_init(config)?;

						apk::cli::build(config, env, noise_level, profile, get_targets_or_all(targets)?, split_per_abi)
							.map_err(Error::ApkError)
					})
				},
			},
			Command::Aab { cmd } => match cmd {
				AabSubcommand::Build { targets, profile: cli::Profile { profile }, split_per_abi } => {
					with_config(non_interactive, wrapper, |config, _, env| {
						ensure_init(config)?;

						aab::cli::build(config, env, noise_level, profile, get_targets_or_all(targets)?, split_per_abi)
