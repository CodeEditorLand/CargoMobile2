	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match &self.cause {
			...
			PromptErrorCause::NoneDetected => {
				write!(f, "Failed to prompt for {} device: No connected devices detected", self.name)
			},
		}
	}
}
...
impl<T: Debug + Display> Reportable for PromptError<T> {
	fn report(&self) -> Report {
		match &self.cause {
			PromptErrorCause::DetectionFailed(err) => Report::error("failed to detect devices", err),
			PromptErrorCause::PromptFailed(err) => {
...
macro_rules! define_device_prompt {
	($func:path, $e:ty, $name:ident) => {
		fn device_prompt<'a>(env: &'_ Env) -> Result<Device<'a>, $crate::device::PromptError<$e>> {
			let device_list =
				$func(env).map_err(|cause| $crate::device::PromptError::detection_failed(stringify!($name), cause))?;
			...
			if device_list.len() > 0 {
				let index = if device_list.len() > 1 {
					...
					.map_err(|cause| $crate::device::PromptError::prompt_failed(stringify!($name), cause))?
				} else {
					0
				};
