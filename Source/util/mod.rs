	...
	fn from_str(v: &str) -> Result<Self, Self::Err> {
		match v.split('.').count() {
			1 => Ok(VersionTriple {
				major: v
					.parse()
					.map_err(|source| VersionTripleError::MajorInvalid { version: v.to_owned(), source })?,
				minor: 0,
				patch: 0,
			}),
			2 => {
				...
				Ok(VersionTriple {
					...
					minor: s
						...
						.unwrap()
						.parse()
						.map_err(|source| VersionTripleError::MinorInvalid { version: v.to_owned(), source })?,
					patch: 0,
				})
			},
			3 => {
...
impl VersionTriple {
	...
	pub fn from_split(split: &mut std::str::Split<char>, version: &str) -> Result<Self, VersionTripleError> {
		Ok(VersionTriple {
			major: split
				.next()
				.unwrap()
				.parse()
				.map_err(|source| VersionTripleError::MajorInvalid { version: version.to_owned(), source })?,
			minor: split
				.next()
				.unwrap()
				.parse()
				.map_err(|source| VersionTripleError::MinorInvalid { version: version.to_owned(), source })?,
			patch: split
				.next()
				.unwrap()
				.parse()
				.map_err(|source| VersionTripleError::PatchInvalid { version: version.to_owned(), source })?,
		})
	}
}
	...
	fn from_str(v: &str) -> Result<Self, Self::Err> {
		match v.split('.').count() {
			1 => Ok(VersionDouble {
				major: v
					.parse()
					.map_err(|source| VersionDoubleError::MajorInvalid { version: v.to_owned(), source })?,
				minor: 0,
			}),
			2 => {
				let mut s = v.split('.');
				...
				Ok(VersionDouble {
					...
					minor: s
						...
						.unwrap()
						.parse()
						.map_err(|source| VersionDoubleError::MinorInvalid { version: v.to_owned(), source })?,
				})
			},
			...
			|_text, caps| {
				...
				let this = Self {
					...
					flavor: caps.name("flavor").map(|flavor| RustVersionFlavor {
						...
						candidate: caps.name("candidate").map(|candidate| candidate.as_str().to_owned()),
					}),
					details: caps
						.name("details")
...
pub fn run_and_search<T>(
	...
	command
		...
		.map(|output| {
			re.captures(&output)
				.ok_or_else(|| RunAndSearchError::SearchFailed { command: command_string, output: output.to_owned() })
...
pub fn get_string_for_group(caps: &Captures<'_>, group: &str, string: &str) -> Result<String, CaptureGroupError> {
	Ok(caps
		.name(group)
		.ok_or_else(|| CaptureGroupError::InvalidCaptureGroup { group: group.to_string(), string: string.to_string() })?
		.as_str()
		.to_owned())
}
	...
	E: From<IE>,
{
	...
	std::env::set_current_dir(working_dir)
		.map_err(|source| WithWorkingDirError::CurrentDirSetFailed { path: working_dir.to_owned(), source })?;
...
pub fn gradlew(config: &crate::android::config::Config, env: &crate::android::env::Env) -> duct::Expression {
