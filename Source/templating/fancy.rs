...
impl FancyPack {
	pub fn parse(path: impl AsRef<Path>) -> Result<Self, FancyPackParseError> {
		...
		let this = Self {
			...
			base: raw
				.base
				.map(|name| Pack::lookup(path.parent().expect("developer error: templates dir had no parent"), name))
				.transpose()
				.map_err(Box::new)
	...
	pub fn resolve(&self, git: Git<'_>, submodule_commit: Option<&str>) -> Result<Vec<&Path>, FancyPackResolveError> {
		...
		if self.path.exists() {
			let mut paths = self
				...
				.as_ref()
				.map(|base| {
					base.resolve(git, submodule_commit.filter(|_| base.submodule_path() == self.submodule_path()))
				})
				.transpose()?
				.unwrap_or_default();
