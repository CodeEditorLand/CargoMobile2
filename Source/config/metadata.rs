...
impl Metadata {
	pub fn load(project_root: &Path) -> Result<Self, Error> {
		...
		let toml_str = fs::read_to_string(&path).map_err(|cause| Error::ReadFailed { path: path.clone(), cause })?;

		let cargo_toml = toml::from_str::<CargoToml>(&toml_str).map_err(|cause| Error::ParseFailed { path, cause })?;
